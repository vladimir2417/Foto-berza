...
if(isset($_POST["getProduct"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			<h3 class='product-name header-cart-item-name'><a href='#'>$pro_title</a></h3>
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	...
	while($row=mysqli_fetch_array($run_query)){
		...
		<div class='product-btns'>
		<button class='add-to-wishlist' tabindex='0'><i class='fa fa-heart-o'></i><span class='tooltipp'>dodaj u listu zelja</span></button>
		
		<button class='quick-view' ><i class='fa fa-eye'></i><span class='tooltipp'>pregled proizvoda</span></button>
		</div>
		</div>
		<div class='add-to-cart'>
		<button pid='$pro_id' id='product' href='#' tabindex='0' class='add-to-cart-btn'><i class='fa fa-shopping-cart'></i> dodaj u korpu</button>
		</div>
		</div>
		</div>
		";
...
if (isset($_POST["Common"])) {
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			<th style="width:7%" class="text-center">Ukupno</th>
			...
			<tr>
			<td><a href="store.php" class="btn btn-warning"><i class="fa fa-angle-left"></i> Nastavite kupovinu</a></td>
			...
			if (!isset($_SESSION["uid"])) {
				...
				<a href="" data-toggle="modal" data-target="#" class="btn btn-success">Placanje</a></td>
				</tr>
				</tfoot>
